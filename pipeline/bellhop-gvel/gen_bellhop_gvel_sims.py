...
def get_dt(ARR):
    ...
    return must_bnc, np.sum(np.abs(delay)*np.abs(A)**2)/np.sum(np.abs(A)**2)
...
# %%
for key in all_arrs:
    with open(key+'.csv','w+') as ff:
        headers = ['index','must_bnc','owtt']
        ff.write(','.join(headers)+'\n')
        
        headers = ['index','must_bnc','arr:owtt']
...
def make_plot(tx_z=20.,rx_z=30.,mode='gvel'):
    ...
    for ii,key in enumerate(all_arrs):
        ...
        tt = np.array([x['arr:owtt'] for jj,x in enumerate(all_arrs[key]) if mask[jj]])
