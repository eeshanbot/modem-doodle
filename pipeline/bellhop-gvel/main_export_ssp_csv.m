...
Cq1 = interp1(OBJ_EOF.depth,OBJ_EOF.baseval,Zq,interpMethod);
writematrix([Zq Cq1],'ssp-artifact-baseval.csv');

Cq2 = interp1(OBJ_EOF.depth,OBJ_EOF.baseval + OBJ_EOF.eofs * artifact_weights,Zq,interpMethod);
...
Cq3 = interp1(OBJ_EOF.depth,OBJ_EOF.baseval,Zq,interpMethod);
writematrix([Zq Cq3],'ssp-fixed-baseval.csv');

Cq4 = interp1(OBJ_EOF.depth,OBJ_EOF.baseval + OBJ_EOF.eofs * fixed_weights,Zq,interpMethod);
writematrix([Zq Cq4],'ssp-fixed-eeof.csv');

%% HYCOM
load hycom-ssp-icex20.mat
Cq5 = interp1(hycomZ(1:end-1),hycomC(1:end-1),Zq,interpMethod);
