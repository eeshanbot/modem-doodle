...
t = tiledlayout(3,7,'TileSpacing','compact');
...
for k = 1:3
    ...
    %% raytrace plot
    nexttile([1 7]);
    [R,Z,~] = eb_raytrace(zs,theta,numstep,sstep,ssp.depth,ssp.ssp,0,max(ssp.depth));
    ...
    title(sprintf('Source depth = %u m',zs),'fontsize',14);
    xlim([0 3.5]);
    ...
    for ne = 1:num_eigentable
        ...
        if strcmp(eigentable{ne}.env,ssp.filestr)
