...
We thank the reviewers and editor for their thoughtful feedback and comments in improving this work. We have significantly restructured the manuscript along several suggestions to separate the real-time navigation methodology, the real-time positioning results (Mininum Bounce Criteria), and the post-processing re-positioning improvement (Nearest Bounce Criteria). We have also introduced that language (navigation, positioning, and re-positioning) more explicitly to avoid confusion and better contextualize our contribution.
...
\section{Reviewer One}
	...
	This summary captures key results presented in the paper. However, we note that the system was successfully deployed on an underwater vehicle during ICEX20. The results from this work are specifically limited to the methodology behind calculating a single group velocity, which can be quantified through beacon-to-beacon communications.
	...
	A robust comparison is tainted by the mismatch of experimental conditions in estimated sound speed environment and source / receiver position.
