...
\section{Reviewer One}
	...
	\lreviewer{Lines 40-42}{}{Note ranging via one way travel times that have been used prior to the advent of the micro-modem, for example RAFOS.}

	\lreviewer{Line 192}{}{Clarify what you mean when you state that the transmit and receive layers are independent? Also, the discussions of physical and virtual layers are confusing. I think everything is there, but reorganization would make this more clear. Consider moving up the paragraph starting at line 203.}

	\lreviewer{Line 205}{}{What do you mean that the coordinate system is range independent?}

	\lreviewer{Line 222}{}{Could you provide more information on the baseline and weighted SSPs.  Where did the baseline profile come from?  }

	\lreviewer{Line 256}{}{How is the group velocity uncertainty calculated?}

	\lreviewer{Line 231}{}{Quantify the travel-time proximity to recorded data that was used to filter the eigenrays.}

	\lreviewer{Lines 260, 303-305}{}{Please provide more information on the BELLHOP implementation.   Line 260 indicates a sparse grid.  From lines 303-305 do I understand that the grid is 1m in range and 2 m in depth?}
	...
	\lreviewer{Line 309}{}{Suggest re-writing this sentence for clarity: “As expected, the algorithm is generally overestimating range as it resolves a direct path that does not represent the actual arrival.”}

	\lreviewer{Line 364}{}{Could use a figure reference.}

	\lreviewer{Line 562}{}{I don’t agree that many approaches to underwater navigation combine it with acoustic tomography.}
	...
	\lreviewer{Figure 6}{}{Suggest including receiver depth along with source depth in the title as they are both important in distinguishing between the cases.}
...
\section{Reviewer Two}
	...
	\lreviewer{Line 7}{}{'Beaufort Lens' This is not yet a scientific term, suggest finding descriptive terms to use. What comprises the 'lens'? The Pacific Summer Water is the warm layer.}

	\lreviewer{Line 10}{}{'GPS aided', do  you mean GPS-navigated?}
	Yes.

	\lreviewer{Line 12}{}{'rival GPS' is a fairly bold statement when GPS is used as ground truth.}
	...
	\lreviewer{Line 89}{}{Is the 'lens' the warm water? Or a combination of layers? What does 'lens' mean acoustically? Normally in ocean acoustics people refer to ducts and layers.}

	\lreviewer{Line 94}{}{Ice Tethered profiler data shows fairly slow variability in space and time in the Beaufort, but changes in characteristics occur as the deep basin transitions to the Chukchi, so what is meant by 'high degree' exactly?}

	\lreviewer{Fig 4 and Line 216}{}{Fig 4 and 216 - where did the baseline come from (exactly)? What database, etc.}

	\lreviewer{Line 224}{}{why did a human have to chose an optimal set of weights? Why not just use the measured SSP, or an average of a few?}

	\lreviewer{Line 231}{}{'abundance'. Is there a more precise way to say this? Bellhop does what it is programmed to…}

	\lreviewer{Line 241}{}{'modem triggering time', do you mean 'detected arrival time'?}

	\lreviewer{Line 249}{}{Use of the word 'range' for the 'navy range' type of use is intermixed with range as in distance. Suggest not using 'range' and in 'navy range' and saying 'nav' or 'comms' system or network.}

	\lreviewer{Line 261}{}{'bring'. Do you mean interpolate?}

	\lreviewer{Line 281}{}{Awkward sentence, either unclear or extremely obvious. "...an acoustic arrival does not always take the direct path from source to the receiver,".}

	\lreviewer{}{}{for both criteria: a graphic that shows the method could be very useful.}

	\lreviewer{Line 319}{}{"This is likely driven by the prominence of the duct." Do you mean: consistency of sound speed within the duct where the rays were trapped and thus traveled? Or something else?}
	...
	\lreviewer{Fig 10}{}{Would be good to have some conclusions in addition to the description.}
