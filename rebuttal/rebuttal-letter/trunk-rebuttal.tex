...
Based on several suggestions, we have restructured the manuscript and separated the real-time navigation methodology (used the Minimum Bounce Criteria, MBC, but is impossible to compare given the lack of GNSS), the real-time positioning results (MBC), and the post-processing re-positioning improvement (Nearest Bounce Criteria). 
...
The rebuttal letter is organized with sections delineated by the review document and email. We look forward to your comments and the opportunity to continue improving this manuscript for eventual publication in JASA. Thank you again for your time and consideration.
...
\section{Reviewer One}
	...
	We have both modified the data presented and tempered the claims in the abstract and throughout the manuscript. The reviewer brings up the important point of transferability: how applicable is the algorithm for normal vehicle operations given its performance on a handful of fixed ranges?
	...
	% A robust comparison is difficult given, as the reviewer points out, the handful of fixed ranges. 
	...
	We believe we have tightened up the introduction and conclusion sections by focusing on this work as a model-aided augmentation to an LBL system deployed in under-ice conditions.
	...
	Lastly, we have removed the line in question, and would point out it was originally written as ``navigation'', not ``positioning''.
...
\section{Reviewer Two}
	...
	We meant ``rival GPS'' in that the downstream accuracy of the acoustic range estimation method is within the precision of GPS. However, we have amended this framing to say that our best performing acoustic method does not introduce significant additional error than is already documented by GPS puck precision, effectively extending single GPS performance into the shallow water column for a small volume of interest. Given more beacons, there is potential to reduce uncertainty in the same way that differential GPS works.
	...
	{\color{blue} Yes. BELLHOP finds all possible eigenrays given the prescribed launch angles whereas only a few or less (sometimes none) are valid for the travel time of interest}.
	...
	{\color{blue} The authors are not sure what the reviewer means by this statement, specifically what ``navy range'' is. We believe we are only using ``range'' to mean distance. Nonetheless, we have adopted this suggestion and use ``navigation'' or ``communication'' system/network instead.}
	...
	{\color{blue} Perhaps both. We have changed this accordingly}
	...
	{\color{blue} We mean prominence, often used in peak finding, which would indicate both the considerable sound speed differential and the nearly horizontal slope to the local maxima.}
...
\section{Comments from the Editor}
	...
	Thank you. The key point of this paper is a model-aided component to the LBL paradigm that we demonstrated in an Arctic environment. Because both reviewers and the editor felt the key points were comparing the two different approaches to determine an effective sound speed for converting travel times to ranges, we have significantly restructured the paper to make the narrative clearer. 

	The discussion of AUV operations and jargon from AUV application contextualizes the model-aided component to the LBL paradigm. However, we agree with the Editor that this emphasis can be distracting when used throughout the manuscript. We have attempted to use the most direct language possible in each section. We hope this revision presents the contributions more clearly and welcome further feedback on this matter. 
	...
	We have now included the information on acoustic parameters in the experiment description, see section REFREF. We apologize that this information was ommitted as the manuscript was adapted from the thesis.

	Similarly, we have addressed the editor's point about the ray geometries in the discussion section, REFREF, but have opted to keep those figures in the supplementary material.
	...
	{\color{blue} Thank you for catching this. We have corrected it and made sure all figurs are referenced in the text directly.}
	...
	{\color{blue} We have talked with colleagues and think ``GPS noise'' is the best term. But given that all reviewers and the editor feel this section is out of place, we have significantly reduced it... OR REMOVE IT?}
	...
	When evaluating our methodology, we have removed qualitative language like ``GPS-like'' or ``rivaling GPS'' in favor of quantitative descriptions of ranging error. {\color{blue}achieves ``x meter'' level accuracy compared to ``y meter'' level accuracy reported by the manufacturer}
	...
	We have removed this generalization.
	...
	{\color{blue}Thank you for pointing this out. We have double checked all references.}
...
\section{Reviewer's Responses to Questions}
	...
	Thank you. We have opted to keep the title the same, as we believe the results are a strong proxy for under-ice acoustic navigation, which we demonstrated during ICEX20 but there are inherently no data products to validate this waypoint by waypoint.
