...
Trilateration of beacon events was done in real-time, on the beacons themselves.
In this work, we further delineate that positioning is in real-time and re-positioning is post-processed.
We have also added re-positioning and re-navigation trilateration results to emulate our system performance given the navigation results have no ground truth GNSS to compare to.
...
We also thank JASA Editorial Office for a generous extension given that the corresponding author started parental leave.
...
\section{Reviewer One}
	...
	This summary captures key results presented in the paper. However, we note that the system was successfully deployed on an underwater vehicle during ICEX20. The results from this work are specifically limited to the methodology behind calculating a single group velocity, which we attempt to validate through beacon-to-beacon communications with known GNSS timestamps.
	...
	We have adjusted the claims made in the abstract (and throughout the paper) to better emphasize what was deployed in the field, as part of normal vehicle operations, and how the proposed improvements compare to the fielded approach. We have presented data that we feel better support the claims about single meter accuracy as well. 
	While we initially aimed to contextualize the calculation of a handful of short fixed ranges between ice-tethered buoys as the proxy for navigation, we agree with the reviewer that this representation of our work is incomplete in portraying its portability to vehicle operations. Accordingly, we added the same analysis for the AUV run in ICEX20. 
	...
	Within the data collected during ICEX20, we actually do see instances of multiple arrivals detected at a single receiver, spread over multiple seconds\textemdash this can be exemplified by arrivals consistent with bottom-bounce paths. 

	The point of the Nearest Bounce Criteria, the improved approach proposed in this paper as part of the post-processing analysis, is precisely to expose how tracking the results of the acoustic model by modal variations can impact the ranging and positioning results; this is done using field data as the foundation. In our case, at short ranges under the Arctic ice, we have opted to track boundary interactions as the basis for modal variation; however, a different arrival classification basis may be needed for longer ranges in the SOFAR channel, for example.
	...
	Thank you, these are all excellent suggestions. Instead of ``group velocity'' we use ``effective sound speed'', noted by $c$. The statement in question has been rewritten to talk about a single sound speed compensating for ray refraction and reflection, and is on line \ref{v1:1.5}.
	...
	We meant that the analysis of beacon-to-beacon connections is presented in r/z, not x/y/z. We found this framing awkward as it is more common to say the propagation has a depth dependent sound speed profile. Range-dependent models may assume no change in the sound speed profile, but account for a range dependent bathymetry; or they may account for changes in both bathymetry and SSP over range. This work includes a range dependent bathymetry.
	...
	Thank you. We have added a successful decoding percentage, defined as the ratio of decoded over detected messages. We have also added the total number of transmissions, so one can see the ratio of detected over transmitted. An event is deemed unsuccessful given any error in the transmitted headers or any data frame, even if the error has nothing to do with the information necessary for positioning.
...
\section{Reviewer's Responses to Questions}
	...
	Thank you. We have significantly reorganized the paper to improve clarity as suggested.
	...
	We have significantly condensed the background for this section. In addition, we narrowed the discussion to the how trilateration error compares to that of GNSS at high latitudes and our particular GNSS unit. 
