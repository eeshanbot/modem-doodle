...
For real-time navigation, this conversion assumes an isovelocity sound speed.
...
This work demonstrates an embedded, ray-based prediction method of the effective horizontal group speed to minimize vehicle position error.
...
\llabel{2.2} Given the lack of GPS data throughout the vehicle's mission, however, the localization performance is quantified between GPS-linked beacon-to-beacon connections where a single beacon ran the vehicle's navigation stack.
The real-time positioning error is roughly 11 meters at distances up to 3 km.
In post-processing, the dataset is extended to all possible beacon-to-beacon events.
The localization performance is reevaluated with an operationally equivalent navigation stack using modeled, historical, and locally observed sound speed profiles for a Minimal Bounce Criteria (MBC), as deployed in real-time, and an improved algorithm, the Nearest Bounce Criteria (NBC).
The best performing median re-positioning errors are roughly 10 and 2 meters, respectively.
\llabel{2.3} Further investigation suggests that the GPS ground truth data have some noise that is not reciprocated in the travel time data.
\end{abstract}
...
While land and air-based robots utilize information from  Global Positioning System (GPS) updates\textemdash or more broadly, any Global Navigation Satellite System (GNSS)\textemdash to achieve stunning location accuracy and precision throughout the duration of their missions, AUVs cannot access GPS while underwater due to the rapid attenuation of electromagnetic waves.
Therefore, underwater vehicle navigation has relied on any combination of dead reckoning, hydrodynamic models, inertial navigation systems, doppler velocity logs, and acoustic baseline positioning systems \citep{paull_auv_2014}.
Limiting navigation error and drift requires an AUV to periodically stall on the surface and obtain a GPS fix to reset its position error.
...
Of acoustic baseline positioning systems, long baseline (LBL) is the most GPS-like in style and scale, and most appropriate for mitigating drift in underwater vehicle localization without overburdening computation or payload size on the vehicle \citep{paull_auv_2014,van_uffelen_global_2021}.
The state-of-the-art for LBL outsources depth to a pressure sensor and assumes an isovelocity, linear scaling between one way travel time (OWTT) and range \citep{eustice_recent_2006,eustice_experimental_2007,webster_preliminary_2009,webster_advances_2012}.
This assumption is valid for small and short scale operations but introduces error in complex acoustic propagation environments.
Thus, this paper demonstrates embedded acoustic model-aided data processing to convert recorded OWTTs into pseudoranges in real-time.
This approach was necessitated by a low risk tolerance for navigation error, given total under-ice conditions and double ducted acoustic environment, in the Beaufort Sea, in March 2020, during the Ice Exercise 2020 (ICEX20).
We quantify the success of the proposed acoustic ranging method on GPS-linked beacon-to-beacon communication events, not the AUV-LBL positioning itself, as the latter has no ground truth GPS to compare to.

In the interest of clarity, we formally reiterate definitions of timing, positioning, and navigation (CITE).
Timing is the ability to maintain accurate and precise time synchronized across multiple nodes.
Positioning is the ability to accurately determine one's location referenced to a standard geodetic system.
It can be done via trilateration or triangulation.
Navigation, implying a degree of autonomy, is the ability to distinguish between current and desired position, and apply necessary corrections to course, orientation, and/or speed to attain a future desired position.
Thus, navigation is also intrinsically real-time and is predicated on positioning, which is predicated on timing.

Given the computational and physical barriers of assessing underwater position in real-time, we define post-processing counterparts\textemdash re-navigation and re-positioning\textemdash which may also be supplemented by additional data, model output, and bespoke algorithms.

\subsection{Long baseline navigation}
LBL navigation was first introduced ...

significantly revamped by ... 

key assumptions are ... 

papers by x,y,z show this much error. generally reserved for small and short scale deployments.

\subsection{Acoustic methods for re-navigation and re-positioning}

The LBL navigation methodology simplifies the acoustic propagation to an isovelocity ocean.
Post-processing methods can take advantage of additional information 
recording acoustic information but not using it onboard.

It is important to note that post-processing localization efforts (re-navigation for a self-determinedly moving object or re-positioning for a passively moving one), are often applied for large-scale deployments to better account for spatial variability in the ocean state.

Recently, buoyancy gliders have been re-navigated by considering a temporally averaged depth dependent SSP and CTD observations of...

A well known example of long range re-positioning are RAFOS drifters that record the timestamps of the four strongest pings; transmit all data after mission completion; and are manually localized with uncertainties of 1.5 to 4.5 km (CITE ROSSBY, DUDA).

Other re-positioning methods employ four-dimensional ocean models.
\citet{wu_deep_2019} compares acoustic records with synthetic acoustic records computed through HYCOM \cite{chassignet_hycom_2007}.
\citet{mikhalevsky_deep_2020} introduces an impressive ``cold start'' algorithm that isolates a specific ray path to find a representative group speed, re-positioning a floating hydrophone array with an error of 85 m and a standard deviation of 32 m relative to ground truth GPS data.
Implementing these algorithms for navigation remains to be seen.

\subsection{Underwater navigation in the Arctic}

\subsection{The New Arctic}

Of particular note are previous AUV deployments in the Arctic that used an LBL navigation system.
A more comprehensive review of AUV deployments with a myriad of applications can be found in xx, yy.

Because model-based acoustic arrival methods have generally been deemed too computationally and/or labor intensive for real-time, 

Underwater navigation research is broadly motivated by acquiring GPS-like navigation accuracy (on the order of meters) in GPS-denied conditions.

Navigation efforts are currently limited to small-scale deployments, and take advantage of 
\llabel{1.11} the advent of the WHOI micro-modem \citep{singh_underwater_2006} and synchronized chip scale atomic clocks \citep{gardner_second_2016}.
Short range (less than ten kilometers) navigation efforts achieve minimal error with a nominal sound speed value \citep{eustice_experimental_2007,webster_preliminary_2009,kepper_mems_2017}.
However, these efforts were all conducted with X sound speed conditions...

Of particular note are previous efforts in the Arctic's Beaufort Lens.
...
The LBL system provides navigation aiding as position updates to correct for localization drift and error.

\subsection{Underwater positioning and navigation in the Arctic}

We refer the reader to excellent reviews of vehicle operations in the Arctic by \citet{norgren_unmanned_2014} and \citet{barker_scientific_2020}.
Previous AUV deployments in the Arctic \cite{bellingham_auv_1993}
...
\llabel{2.4} The Beaufort Lens is a highly dynamic, highly stratified water mass, keeping a neutrally buoyant layer of warm Pacific water at 50 to 70 m in depth.
...
\llabel{2.5} Modeling output \citep{duda_long-range_2019,duda_effects_2021} and experimental observations \cite{kucukosmanoglu_observations_2021,badiey_azimuthal_2019,ballard_temporal_2020} indicate a high degree of spatio-temporal variability of the lower duct throughout the Chukchki and Beaufort Seas.
...
During ICEX20, we deployed AUV \emph{Macrura}, a custom Bluefin-21, in complete under-ice conditions and a complex double ducted environment brought upon by a persistent warm water intrusion in the Pacific Summer Water layer, which we call the Beaufort Lens.
The majority of the navigation updates were below 11 m and were small enough to enable an emergency recovery. 
When considering operational variables such as range, vehicle class, and propagation conditions, there are no comparable deployments.
...
\item \llabel{1.12} The transmit and receive layers are independent
...
\llabel{1.13} A key construction in the ICNN are parallel physical and virtual layers; given the small operational scale, on the order of kilometers, and a singular SSP, both layers are projected into a range independent coordinate system for ease of comparison.
...
\llabel{1.14} During ICEX20, the SSP was toggled between the historical``baseline'' and a ``weighted'' estimate based on a local CTD cast, i.e. an optimal set of weights chosen by a human decision maker \llabel{2.7} via an interactive Tactical Decision Aid framework \citep{bhatt_embedded_2021}.
\llabel{1.8} For the post-processing analysis, we add the SSP from HYCOM for comparison, to mirror the information available on a submarine\textemdash modeled data, historical data, and \textit{in situ} data (personal conversation with LT B. Howard and LT CDR D. Goodwin).
...
\llabel{1.16} It is important to note that BELLHOP produces an \llabel{2.8}abundance of eigenrays; the ones displayed here are chosen by travel time proximity to the recorded data.
...
\llabel{2.9} For the Beaufort Lens in particular, the strong multipath effects make it virtually impossible to deterministically predict the modem triggering time.
...
\llabel{2.10} In the ICEX20 configuration, the acoustic tracking is running on the topside computer, which controls the integrated communication and navigation range.
...
\llabel{2.11} The initial call to BELLHOP is over a sparse local grid, from which the plane wave approximation is used to interpolate the ray to the range and depth posited by the onboard tracking solution.
...
\llabel{2.12} As shown in the eigenray traces of Fig. \ref{fig:raytrace-zs30}, an acoustic arrival does not always take the direct path from source to the receiver, and the path it takes is extremely dependent on the sound speed profile.
...
\llabel{1.17} In addition, for a more consistent comparison, the local sparse grid is standardized as 11$\times$11 points over 10 m in range and 20 m in depth, which is similar to those from the vehicle logs.
...
\llabel{1.18} As expected, the algorithm is generally overestimating range as it resolves a direct path that does not represent the actual arrival.
...
\llabel{2.14} This is likely driven by the prominence of the duct.
...
% The results presented herein are from post- processed analysis, however they use only the data that was available in real-time—raw navigation data and only the acoustic transmissions that were successfully decoded by the AUG in real time.
% The reasons for the difference in performance are most likely due to propagation conditions in and out of the duct. Receivers in the duct hear the source at any range, while those outside the duct are in a range-dependent environment where signals come and go depending on the ray paths.
