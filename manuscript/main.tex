...
The state-of-the-art for LBL assumes an isovelocity, linear scaling between one way travel time (OWTT) and range \citep{eustice_recent_2006,eustice_experimental_2007,webster_preliminary_2009,webster_advances_2012}.
This assumption is valid for short scale operations but introduces error in complex acoustic propagation environments.
...
Positioning is the ability to accurately determine one's location referenced to a standard geodetic system. It can be done via trilateration or triangulation.
Navigation, implying a degree of autonomy, is the ability to distinguish between current and desired position, and apply necessary corrections to course, orientation, and/or speed to attain that desired position.
Thus, navigation is also intrinsically real-time and is predicated on positioning, which is predicated on timing.

Given the computational and physical barriers of assessing underwater position in real-time, we define post-processing counterparts\textemdash re-navigation and re-positioning\textemdash which may be supplemented by additional data, model output, and computationally or labor intensive algorithms.

\subsection{Background on underwater navigation, re-navigation, and re-positioning}
