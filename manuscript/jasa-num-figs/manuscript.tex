...
\author{EeShan C. Bhatt \footnote{Also at MIT-WHOI Joint Program in Oceanography/Applied Ocean Science \& Engineering, Cambridge, MA}}
...
\llabel{2.2} Given the lack of GNSS data throughout the vehicle's mission, the pseudorange performance is first evaluated on connections between GNSS-linked beacons.
...
An operationally equivalent pipeline is used to re-position the LBL beacons and re-navigate the AUV, using modeled, historical, and a locally observed sound speed profiles.
The median re-navigation error is 1.84 $\pm$ 2.19 m.
\llabel{2.3} The improved performance for suggests that this approach extends the single meter accuracy of the deployed GNSS units into the water column.
\end{abstract}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\columnwidth]{Fig2a.pdf} \hfill
    \includegraphics[width=0.28\columnwidth]{Fig2b.jpg}
...
The exact adjustments to the ensemble eigenray filtering are predicated on the expected sound speed conditions and acoustic arrival structure; the problem is ripe for other simulation testbeds or machine learning methods.
